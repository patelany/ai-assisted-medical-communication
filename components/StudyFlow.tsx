...
export default function StudyFlow() {
  ...
  const generateScenarioMessages = async () => {
    ...
    // Use pre-generated if available
    ...
    // Fallback to fetching if pre-generation failed
    try {
      const res = await fetch("/api/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  if (step === "clinician-form") {
    return (
      <div className="flex flex-1 min-h-screen">
        {/* LEFT SIDEBAR */}
        <div className="w-80 min-w-80 bg-white border-r border-gray-100 flex flex-col gap-5 overflow-y-auto p-6 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-300 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-gray-400">
            ...
            <div className="bg-white rounded-2xl p-6 max-w-sm w-full flex flex-col gap-4 shadow-xl">
              ...
              <div className="flex flex-col gap-2">
                <button
                  ...
                  className="w-full bg-gray-900 text-white rounded-xl py-3 text-sm font-medium hover:bg-gray-700 transition-colors cursor-pointer"
                >
                  Continue to rating
                </button>
                <button
      ...
      <div className="max-w-xl mx-auto py-10 px-4 md:px-6 pb-24">
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-6 py-4 flex items-center justify-between gap-4 z-50">
          ...
          <button
            ...
            className="relative bg-gray-900 text-white rounded-xl px-6 py-2.5 text-sm font-medium hover:bg-gray-700 transition-colors cursor-pointer flex-shrink-0 overflow-hidden"
          >
        ...
        <div className="mb-6">
          ...
          <div className="h-1 bg-gray-200 rounded-full overflow-hidden">
            <div className="h-full bg-gray-900 rounded-full transition-all duration-500"
              style={{ width: `${(currentScenarioIndex / scenarioOrder.length) * 100}%` }} />
          </div>
        </div>
