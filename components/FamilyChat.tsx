...
export default function FamilyChat({ currentUpdate }: FamilyChatProps) {
  ...
  return (
    <>
      ...
      {/* CHAT WINDOW */}
      {open && (
        <div className="fixed bottom-24 right-6 w-80 bg-white border border-gray-200 rounded-2xl shadow-2xl flex flex-col overflow-hidden z-50"
          style={{ height: "420px" }}
        >
          {/* HEADER */}
          <div className="bg-emerald-700 text-white px-4 py-3 flex flex-col gap-0.5 flex-shrink-0">
          ...
          <div className="bg-amber-50 border-b border-amber-100 px-3 py-2 text-xs text-amber-700 flex-shrink-0">
            ⚠️ Not a substitute for medical advice. Contact the care team for clinical questions.
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3 flex flex-col gap-3">
            {messages.map((m, i) => (
              ...
              >
                <div
                  className={`max-w-64 rounded-2xl px-3 py-2 text-xs leading-relaxed ${
                    m.role === "user"
          ...
          <div className="border-t border-gray-100 p-3 flex gap-2 flex-shrink-0">
            <input
              ...
              className="flex-1 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-emerald-600"
            />
            <button
              onClick={handleSend}
              disabled={loading || !input.trim()}
              className="bg-emerald-700 text-white rounded-lg px-3 text-xs font-semibold hover:bg-emerald-800 transition-all disabled:bg-gray-200 disabled:cursor-not-allowed"
            >
              Send
            </button>
          </div>
